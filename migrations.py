...
class Migrate_creater:
    def __init__ (self, app_name, model_name):
        ...
        self.file_path = '{}/{}/models.py'.format(setting.DIR_WITH_DJANGO,self.app_name)
    ...
    def __find_field(self, field_name):
        ...
        if len(start_pos_list) == 0:
            ...
            corr_start_pos_list = [start_pos_list.index(x) for x in start_pos_list if re.search('class {} '.format(self.model_name), all_file[x]) 
        ...
        field_pos_list = [model_list.index(x) for x in model_list  if re.search('{}.+=.+models|{}=models|{}.+=models|{}=.+models'.format(field_name,field_name,field_name,field_name), x)]
